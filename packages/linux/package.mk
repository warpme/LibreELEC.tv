...
case "$LINUX" in
  ...
  hverkuil)
    PKG_VERSION="3c6248f"
    PKG_URL="https://git.linuxtv.org/hverkuil/media_tree.git/snapshot/$PKG_VERSION.tar.xz"
...
make_target() {
  ...
  if [ "$BUILD_ANDROID_BOOTIMG" = "yes" ]; then
    LDFLAGS="" mkbootimg --kernel arch/$TARGET_KERNEL_ARCH/boot/$KERNEL_TARGET --ramdisk $BUILD/image/initramfs.cpio.gz \
